    ...
    with st.form("bill_form", clear_on_submit=True):
        ...
        if btype=="ITEMS":
            ...
            if rtype=="Normal Repair":
                b=st.number_input("Base repair charge (₹)", min_value=0.0, key="user_normal_repair")
                total=b+LABOR
                det=f"Normal Repair: ₹{b}+₹{LABOR}"
            else:
                p=st.number_input("Number of parts repaired", min_value=0, step=1, key="user_adv_repair_parts")
                total=p*PART_COST
                det=f"Advanced Repair: {p}×₹{PART_COST}"
        else:
            c_amt=st.number_input("Base customization amount (₹)", min_value=0.0, key="user_custom_amt")
            total=c_amt*2
    ...
    if menu == "Sales":
        ...
        with tabs[0]:
            ...
            with st.form("add_emp", clear_on_submit=True):
                ...
                new_rank = st.selectbox("Rank", list(COMMISSION_RATES.keys()), key="add_emp_rank")
        ...
        with tabs[2]:
            ...
            if details:
                with st.form("edit_emp", clear_on_submit=True):
                    name = st.text_input("Name", details["name"], key="edit_emp_name")
                    rank = st.selectbox(
                        "Rank",
        ...
        with tabs[0]:
            ...
            if not emp_keys:
                ...
                view = st.radio("View", ["Overall", "Detailed"],
                                horizontal=True, key="tracking_emp_view")
        ...
        with tabs[3]:
            ...
            view = st.radio("Show", ["Active","Past"],
                            horizontal=True, key="tracking_mem_view")
            if view == "Active":
        ...
        with tabs[5]:
            ...
            days = st.number_input("Last X days", min_value=1, max_value=30, value=7, key="tracking_filter_days")
            min_sales = st.number_input("Min sales amount (₹)", min_value=0.0, value=0.0, key="tracking_filter_min")
